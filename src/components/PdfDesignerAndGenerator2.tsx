...
const PdfDesignerAndGenerator2: React.FC = () => {
  ...
  // Initialize or reinitialize the Designer
  useEffect(() => {
    ...
    // Set a small delay to ensure DOM is ready and previous instance is fully cleaned up
  ...
  // Initialize or reinitialize the Form when showing it
  useEffect(() => {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Ensure designer is properly destroyed before changing template
      if (designerInst.current && !isDesignerDestroyed) {
        try {
          designerInst.current.destroy();
          designerInst.current = null;
  ...
  // Handle transition to form view
  const handleShowForm = () => {
    ...
    try {
      ...
      // Store template before switching views
      setShowForm(true);
    } catch (err: any) {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      // Get template from designer (safely)
      let template;
      if (designerInst.current && !isDesignerDestroyed) {
  ...
  // Handle transition back to designer
  const handleBackToDesigner = () => {
    // Ensure form is properly destroyed
    if (formInst.current && !isFormDestroyed) {
      try {
        formInst.current.destroy();
        formInst.current = null;
...
//       // Create URL for displaying the PDF
//       if (currentPdfUrl) {
